...
export class CronService {
  ...
  public static getInstance(bot?: Bot, chatId?: string | number, staffService?: any, orderService?: any): CronService {
    if (!CronService.instance) {
      ...
      CronService.instance = new CronService(bot, chatId, staffService, orderService);
    }
  ...
  async checkProductionStatus() {
    try {
      ...
      for (const { order, item } of itemsToCheck) {
        ...
        const questionMsg = t("followup_question", workerLang, {
          customer: order.customerName,
          product: item.product,
          quantity: String(item.quantity),
        });
        ...
        const keyboard = new InlineKeyboard()
          .text(
            t("btn_yes_done", workerLang),
            `production_done:${item.id}`,
          )
          .text(
            t("btn_no_ongoing", workerLang),
