...
import { SelfCleanupService } from "../services/self-cleanup.service.js";
...
export class CommandHandler {
  ...
  public async handleDev(ctx: Context) {
    ...
    const response = await llm.chat({ userMessage: query, context: technicalPrompt });
    await ctx.reply(
  ...
  public async handleTemizlik(ctx: Context) {
    ...
    try {
      ...
      await supabase.resetOrdersAndVisualMemory();
    } catch (e: any) {
      dbError = e.message;
    }
